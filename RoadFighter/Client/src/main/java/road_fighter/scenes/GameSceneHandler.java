...
public class GameSceneHandler extends SceneHandler {
	...
	public GameSceneHandler(Main g) {
		super(g);
		gameUtil = new GameUtil(main.client);
	}
	...
	protected void defineEventHandlers() {
		keyEventHandlerPressed = new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent e) {
				...
				case W:
				case UP:
					Mensaje mensaje = gameUtil.crearMensaje(Comandos.MOV_ARRIBA);

					main.client.enviar(mensaje);
					break;
				case DOWN:
	...
	public void load(boolean fullStart) {
		...
		this.contrincantes = new ArrayList<AutoJugador>();

		this.background = new Background("file:src/main/resources/img/bg3.png");
		this.score = new Score();
		this.pista = new Pista();
		this.habilidad = new Habilidad();
		this.autoJugador = new AutoJugador(this.main.client.getUsername(), 540, 720, this.score, this.habilidad);
		this.main.client.setUser(this.autoJugador);

		for (int i = 0, j = 0; i < this.main.client.getPlayersInSala().size(); j += 40, i++)
			this.contrincantes.add(new AutoJugador(this.main.client.getPlayersInSala().get(i), 540 + j, 720, this.score,
					this.habilidad));

		for (int i = 0; i < this.main.client.getPlayersInSala().size(); i++) {
			if (this.contrincantes.get(i).getUsername() == this.autoJugador.getUsername()) {
				this.contrincantes.remove(i);
			}
		}
		...
		gameOB.add(this.background, this.pista, this.autoJugador, this.objetoBuilder, this.meta, this.score, this.gRi,
				this.gRd, this.habilidad);

		for (int i = 0; i < this.contrincantes.size(); i++)
			gameOB.add(this.contrincantes.get(i));
