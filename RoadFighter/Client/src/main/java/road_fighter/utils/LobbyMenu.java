...
public class LobbyMenu extends Menu implements Renderable {
	...
	public LobbyMenu(Client client) {
		...
		scroll.setLayoutX(Config.anchoBase - X);
		scroll.setLayoutY(Y);
		...
		Rectangle bottomBorder = new Rectangle(0, Config.altoBase - FONT_SIZE, Config.anchoBase, FONT_SIZE);

		render = new Pane(salasBox, scroll, bottomBorder, deleteText,
				info2("c - Crear Sala\nENTER - Unirse en Sala", 2));
	}
	...
	public Mensaje eliminarSala() {
		Mensaje msg = new Mensaje(Comandos.ELIMINAR_SALA);
		msg.agregar(texts[focus].getText());
		return msg;
	}

	public Mensaje unirseSala() {
		Mensaje msg = new Mensaje(Comandos.UNIRSE_SALA);
